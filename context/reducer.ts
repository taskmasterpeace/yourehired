...
function addOpportunityToEvents(events: CalendarEvent[], opportunity: Opportunity): CalendarEvent[] {
  ...
  switch (action.type) {
    ...
    case 'DELETE_OPPORTUNITY': {
      ...
      try {
        ...
        const opportunityExists = state.opportunities.some(opp => opp.id === opportunityId);
        if (!opportunityExists) {
          console.warn(`Attempted to delete non-existent opportunity with ID: ${opportunityId}`);
          return state; // No changes if opportunity doesn't exist
        }
        
        // Filter out the deleted opportunity
        const updatedOpportunities = state.opportunities.filter(opp => opp.id !== opportunityId);
        
        // Filter out related events
        const updatedEvents = state.events.filter(event => event.opportunityId !== opportunityId);
    ...
    case 'UPDATE_EVENT': {
      ...
      const updatedEvents = state.events.map(event => {
        ...
        const eventId = typeof event.id === 'string' ? parseInt(event.id as string, 10) : event.id;
        const updateId = typeof id === 'string' ? parseInt(id as string, 10) : id;
    ...
    case 'DELETE_EVENT': {
      ...
      const idToDelete = typeof action.payload === 'string' ? 
      ...
      const newEvents = state.events.filter(event => {
        ...
        const eventId = typeof event.id === 'string' ? 
          parseInt(event.id as string, 10) : event.id;
